...
**放弃"物理结构提升性能"的旧故事(已被 5 种注入方式 × 2 种 init × 3 域 + 真实数据全线证伪),转投"系统性解剖"论文**:以"**可解码 ≠ 预测依赖它**(presence ≠ use)"为统一论点,讲清(1)什么真的有效(free-rollout / horizon 匹配 / 域匹配增广),(2)物理结构先验为什么失效(表示挤占 + 梯度冲突,机制有证据),(3)评测怎么骗人(cos 陷阱 / 迁移天花板 / 协议混淆)。这是诚实、完整、有机制解释的实证科学论文,数据量足以支撑 AAAI。
...
- **2026-07-13:补强"负结果非实现 bug"的论证**(应对审稿人"你实现有 bug / 换编码方式会不会好")。跑了 **probe-190 实验**(新脚本 `phyworld/scripts/probe_dim_subset.py`):三域实测 **黑盒 190 维独立冗余编码位置(ρ 0.79–0.92),即便钉了 slot 也没削弱 → 旁路实测存在**,把机制核心从"定性断言"升级为"三域实测"。并处理了两个精细驳斥点(注入量不够 / 更优雅融合),给出"旁路二分"framing 和 claim 精确措辞。**全部记录在 [detail/why_physics_structure_fails.md](detail/why_physics_structure_fails.md)**。extrinsic 突破仍零结果(主会话 PIWM 移植卡在 VAE 阶段)→ future work。**待定:论文正文 abstract/§4.5/conclusion 是否按"旁路"边界微调 claim 措辞(建议措辞已在 detail,正文暂未改,等拍板)。**
