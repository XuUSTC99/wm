#!/bin/bash
# queue6 (2026-07-16) CRITICAL: pw300 on dinowm/uniform gave a SIGNIFICANT 0.67x
# (3 seeds, std 0.015, error bars disjoint from baseline). If this holds on
# collision+parabola too, it's positive validation of the bypass mechanism:
# freezing the encoder makes the projector a bottleneck -> the slot becomes
# load-bearing -> injection finally works. Test all 3 domains x 3 seeds.
# Also probe whether it's pos_weight per se or the frozen bottleneck: run pw300
# on LeWM-style trainable encoder for contrast is already in the paper (harmful).
set -u
...
LOG=/data1/likun-share/junjxu/runs/dinowm; QLOG=$LOG/queue6.log; mkdir -p "$LOG"
...
for s in 3072 1234 42; do
  Q+=("dinowm_col_structpos_pw300_s${s}|$COL|collision|$SLOT loss.pos_weight=300 seed=$s")
  Q+=("dinowm_par_structpos_pw300_s${s}|$PAR|parabola|$SLOT loss.pos_weight=300 seed=$s")
done
# pw1000: does the gain keep growing (dose-response = mechanism evidence)?
for s in 3072 1234 42; do
  Q+=("dinowm_um_structpos_pw1000_s${s}|phyworld_uniform_motion_id1k|uniform_motion|$SLOT loss.pos_weight=1000 seed=$s")
done
run_one() {
...
echo "=== queue6 START $(date), ${#Q[@]} jobs ===" > "$QLOG"
i=0
for item in "${Q[@]}"; do
...
wait
echo "=== queue6 ALL FINISHED $(date) ===" >> "$QLOG"
