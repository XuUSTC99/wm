...
2. 物理注入**不是"作用不大",是系统性有害**(27 格里 15 格明确变差、11 格噪声内、26/27 不优于 baseline、从头共训 Δ+0.558)——反直觉点就在"有害",别软化。
...
4. **答案预览 + 贡献**:①(负,主体)注入**系统性有害**——27 格里 26/27 不优于 baseline,连正确物理形式(a=g)和从头共训都救不回;②(机制)因为 latent **已经**冗余编码了物理状态,注入 = 塞副本 + 抢梯度;干预证明预测**确实读**这个 slot,但读的是副本、给不了新信息(**load-bearing but redundant**),并给出可证伪验证;③(对照落点)同一代码同一指标下,**训练协议**(free-rollout/horizon)带来 2.2–8.3× 提升 → 短板在预测器长程动力学、不在状态;④ extrinsic 架构(PIWM 移植)解决承重但不解决编码器-OOD,必要非充分。
...
### 3.1 物理注入设计空间:5 家族 → 10 臂(判决表 27 格的"方案侧")
...
### 4.1 注入全扫结果(↔ 3.1):26/27 不优于 baseline

**总判决**:**27 格**(10 臂跑满,其中 a=g 与 grounded 是同一配置跑两次,合并后 **9 臂 × 3 域**)——**15 格明确变差、11 格落在种子噪声内、仅 1 格真小赢 → 29/30 不优于 baseline**;整列 collision(1.33–1.66×)最狠。
...
- **照片级仿真(换数据集)**:Physion++ 上 structpos/cons/consacc 的逐场景 rollout nMSE 全部差于纯 FR(如 mass_dominoes 0.058 → 0.45/0.59/0.60,**3–10×**;单种子但差距远超种子噪声带;口径 = per-scenario 聚合,源 `raw_data/runs/physionpp/eval_pp_{struct,cons,consacc}_e20.log`)。⚠️ **probe 族在 Physion++ 上未跑**——已请另一 session 补(P0,见 [NOTE_to_physion_session_gaps.md](NOTE_to_physion_session_gaps.md));补齐前,"照片级仿真同样成立"的措辞只覆盖 slot 与 consistency 两族。
   ...
   > **Fig 16 读图**(主表):图上 **9 行**(a=g 与 grounded 为同一配置的两次抽样,已合并;原始 10 臂逐行数值见 detail)= 注入臂(按 5 家族分组:`[slot]`固定编码/`[probe]`深监督/`[dyn]`运动学头/`[cons]`一致性/`[free]`无标签);3 列 = 三域判决分区。每格 = **nMSE/baseline 倍数**(括号内原始 nMSE);**颜色 = 判决:红更差、白持平、绿更好**;`†` = 三种子均值(其余单种子),`✓` = 唯一真提升。→ **几乎全红**;30 格 **25 差 / 4 平 / 仅 posvel·parabola 0.76× 真降 = 29/30 不优于 baseline**。
...
| ② **注入扫描**(合并后 9 臂×3 域=27 格) | **26/27 ≥ baseline**(3 差/23 平/1 好,那 1 格仅差 0.001);dyn a=g、grounded 在 collision **1.27×** 最害;3 个"好"格全在 uniform | ✅ **注入不是通用杠杆**,与 Fig16 的 29/30 一致 |
...
同一 latent、同一代码、同一指标:注入"已有的状态"26/27 无效且有害(4.1),因为注入的是副本(4.2);修"真缺的长程动力学"2.2–8.3×(4.3);去掉副本的 extrinsic 必要非充分(4.4)。→ *decodable but not load-bearing*。

---

## C5 第五章 Conclusion(~0.5 页)

1. **结论**:物理状态在 latent 世界模型里**可解码但不承重**;往共享 latent 注入已有状态系统性有害(26/27);真正的杠杆是训练协议(修长程动力学);extrinsic 架构必要非充分。
...
> **时间紧只讲 3 张**:**Fig 1**(钩子)→ **Fig 16**(27 格全废,主体)→ **Fig 2**(协议才是杠杆)。

| 图 | 讲什么 | 论文位置 | 文件 |
|---|---|---|---|
| **Fig 1** | 可解码 ≠ 预测依赖它(teaser) | §1 Intro | `fig1_thesis_presence_not_use.pdf` |
| **Fig 16** | 注入全扫(9 臂×3 域=27 格),26/27 不优于 baseline | §4.1 主表 | `fig16_physics_injection_scan.pdf` |
