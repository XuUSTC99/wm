...
> **物理信息"存在"于世界模型的 latent 里(可解码),但预测并不**依赖你注入的那一份**(*not load-bearing*)——黑盒维度早已冗余编码了同一份位置,预测绕过注入的物理 slot、走黑盒那份(probe-190 三域实证)。于是往共享 latent 上嫁接物理**状态**只是塞冗余:不带新信号(不提升)、反占表示容量与梯度(有害);真正让模型遵守物理的是训练协议,不是结构先验。**
...
**回答标题两半(一句话精准版,直接对应 Why Doesn't / What Does)**:

> **Why injecting physics doesn't help**:latent 已经把物理**状态**编码得很强(位置 ρ 0.9、冗余分布在黑盒 190 维),注入同一份状态是**冗余**的——不带新信号(不提升)、还占容量分梯度(有害)。真正的短板不在"状态",而在"**预测器的长程 rollout 动力学**"(状态可解码、但一自回归就漂)。
>
> **What does help**:改**训练协议**——free-rollout 通过让预测器在训练时暴露于**自身累积误差**、学会纠偏(修 exposure bias),直接改善长程动力学预测。它**不注入任何物理先验、不碰 latent 结构**,却比一切物理结构注入都管用。
...
2. 物理注入**不是"作用不大",是系统性有害**(21 格 20 格变差、从头共训 Δ+0.558)——反直觉点就在"有害",别软化。
3. 我们**否定的是注入"latent 已有的状态"**,不是所有物理注入;注入 latent **真缺的东西**(动力学/守恒量)或走 **extrinsic 架构**是未证实的开放出路(唯一疑似正例 parabola 速度 −0.026,不倚重)。

**记法**:doesn't help = 补 latent **已有的**(状态,冗余);what helps = 修 latent **真缺的**(长程动力学),而且靠**训练协议**修、不靠灌物理知识。

---

## 逻辑链(每一步承接上一步)

1. **现象**:**未注入任何物理**的 JEPA 世界模型,编码器本身就把位置编进了 latent——真实帧 probe 可解码 ρ(pos 两维,baseline REAL-emb):uniform **0.94–0.98**、parabola 0.70–0.95、collision 0.60–0.89;单步预测也近乎完美(cos 0.98–0.99)。但一自回归 rollout 就崩(collision h28 cos 掉到 **0.24**、OOD 更崩),而位置信息仍留在真实表示里(collision both-OOD REAL-emb ρ≈0.84,两维均)。→ **状态可解码,不代表物理可遵守。**（📊 [Fig 1](figures/fig1_thesis_presence_not_use.png);出处 `aaai_p0/rollout_{域}_baseline_fr_s1234.log` 的 `probe applied to REAL embs` 段）
...
4. **我们的做法**:一次把设计空间扫满——**5 个注入机制家族(含变体共 10 臂;10 臂 × 3 域 = 步5 的 30 格)× 2 种训练方案 × 3 个物理域 × 2 个照片级仿真基准**,种子受控,**~200 个训练 run**(实数 199 个训练日志:structdyn 118/aaai_p0 29/physionpp 31/pretrain 12/rerun 9;旧文 ">60" 系低估)。
