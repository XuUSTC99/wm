...
The appearance recipe reverses on realistic simulation: the same jitter that halves synthetic OOD error degrades Physion++ friction-collision prediction by two orders of magnitude (nMSE $0.062\!\to\!6.44$) while the \emph{aggregate} long-horizon cosine improves (Trap 1 below), and it does not help zero-shot transfer either ($0.597$, below the $0.607$ random-prior ceiling). The boundary is principled: appearance jitter encodes an appearance-invariance assumption that holds under PhyWorld's simple renderer but breaks where appearance carries physical information (friction, mass, material). Scale jitter, which assumes only size-invariance, keeps helping at realistic scale (Appendix~\ref{app:injection}); appearance jitter flips sign. Augmentation gains are domain- and type-specific, not portable.
