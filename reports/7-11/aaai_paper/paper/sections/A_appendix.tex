...
All PhyWorld runs: AdamW, lr $5{\times}10^{-5}$ (from-scratch reruns: $2{\times}10^{-5}$), weight decay $10^{-3}$, bf16, gradient clip $1.0$, $20$ epochs; the from-scratch cells of the pretraining-regime $2{\times}2$ train longer and at a lower learning rate so their baselines converge cleanly---$60$ epochs on uniform motion and collision, $120$ on parabola, which needed the extra budget---with injection on and off sharing the identical budget within each domain, so every $\Delta$ in Table~\ref{tab:scratch} is a within-domain comparison. Batch size $64$ for free rollout ($H{=}8$), $128$ for teacher forcing ($H{=}1$), $48$ for $H{\geq}16$. History size $3$; embedding dimension $192$; SIGReg weight $0.09$. Seeds: $3072/1234/42$ where seed-replicated. Physion++ runs use the readout split ($800$ clips) with the same optimizer.
