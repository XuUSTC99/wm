...
\ \ slot (structpos) & pin dims $[0{:}2]$ to the true position (the object's $x,y$ coordinates; a fixed index range, not an attention slot \citep{locatello2020slot}): the most direct state injection; tests whether a hard constraint helps \\
\ \ \ $+$reweight & raise the slot's share of the prediction loss ($w{=}30$, Eq.~\ref{eq:loss}): if failure is due to the slot's ${\sim}1\%$ gradient share, this should rescue it \\
