...
# Usage: sweep_three_domains.sh "0 1 2 3 4 5 6 7"
set -u
GPUS=(${1:-0})
...
# 27 configs: (domain:datacfg:vel_col:prefix) × (weight) × (frames)
DOMAINS=(
...
# Map domain → eval --domain argument
eval_dom () {
  case "$1" in
...
run_one () {  # gpu domain datacfg vel_col prefix weight frames
...
# Launch all 27 in parallel, round-robin across GPUs
NG=${#GPUS[@]}
